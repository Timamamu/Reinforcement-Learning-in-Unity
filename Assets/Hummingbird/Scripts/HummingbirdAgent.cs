...
public class HummingbirdAgent : Agent
{
    ...
    public override void Initialize()
    {
        ...
        if (flowerArea == null) flowerArea = FindObjectOfType<FlowerArea>();
    ...
    private void UpdateNearestFlower()
    {
        ...
        foreach (Flower flower in flowerArea.Flowers)
        {
            ...
            float dCur = Vector3.Distance(nearestFlower.transform.position, beakTip.position);
            if (dNew < dCur) nearestFlower = flower;
        }
    }
    ...
    {
        if (!collider.CompareTag("nectar")) return;

        Vector3 closestPointToBeakTip = collider.ClosestPoint(beakTip.position);
        if (Vector3.Distance(beakTip.position, closestPointToBeakTip) >= BeakTipRadius) return;

        // Look up the flower for this nectar collider (null-safe)
        ...
        if (trainingMode)
        {
            float bonus = .02f * Mathf.Clamp01(Vector3.Dot(transform.forward.normalized, -flower.FlowerUpVector.normalized));
            AddReward(.01f + bonus);
        }
